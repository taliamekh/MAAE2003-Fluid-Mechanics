...
**1.4 Pressure** — Absolute/gage/vacuum pressure formalized (p_abs = p_gage + p_atm, p_abs = p_atm - p_vac), hydrostatic pressure in oceans (P = Pa + γ_water·h) and atmospheres (P = Pa - γ_air·b) with sign convention (z positive upward, depth h positive downward), equal-pressure horizontal-plane rule with same-fluid caveat, stacked-fluid layers (P1 = P_atm + Σρ_i·g·h_i), multi-fluid manometer technique (jump across at equal-level interfaces, intermediate pressures cancel), 3 embedded figures

### Ch 2 — Hydrostatic Forces & Buoyancy

**2.1 Forces on Plane Surfaces** — Resultant force on horizontal surface (F = γHA, uniform pressure, CP=CG), resultant force on inclined surface derivation (F = ∫p dA → F = p_CG·A = (pa + γh_CG)A), centroid definition (ξ_CG = (1/A)∫ξ dA), ξ = h/sinθ coordinate along incline, center of pressure derivation (moment balance about CG: F·y_CP = ∫y·p dA → y_CP = -γsinθ·I_xx/(p_CG·A)), horizontal CP offset (x_CP = -γsinθ·I_xy/(p_CG·A)), CP always below CG (negative y_CP), gage pressure simplification (pa=0: F=γh_CG·A, y_CP=-I_xx·sinθ/(h_CG·A)), area moments of inertia (rectangle: I_xx=bL³/12, I_xy=0; circle: I_xx=πR⁴/4, I_xy=0), symmetric shapes have x_CP=0, 2 worked examples: Ex 2.1 (seawater gate 5×10ft inclined, F=38400lbf, l=0.417ft, P=29300lbf, Bx=6300lbf, Bz=30700lbf), Ex 2.2 (vertical gate AB 5×4ft, h=9.5ft, F=9360lbf, l=0.178ft, Bx=5100lbf, Ax=4260lbf), problem-solving strategy (5 steps), 4 embedded figures, formulas #19-#20
...
| 8 | 2026-06-10 | Ch 1.4 Pressure + Ch 2.1 Hydrostatic Forces | Added two lectures from uploaded slide PDFs. Ch 1.4 (Notes_Pressure.pdf): abs/gage/vacuum pressure formalized, ocean/atmosphere pressure distribution with sign convention, equal-pressure horizontal-plane rule, stacked-fluid layers, multi-fluid manometer technique — cross-references Ch 1.3 where topics overlap, 3 embedded figures. Ch 2.1 (Hydrostatic Forces on Plane Surfaces): full derivations of resultant force (F=p_CG·A) and center of pressure (y_CP=-γsinθ·I_xx/(p_CG·A)), gage pressure simplification, area moments for rectangle and circle, 2 fully-worked gate examples (seawater inclined gate + vertical gate with hinge/stop), problem-solving strategy, 4 embedded figures. Split Ch 2 into per-lecture files (ch2-lec1/2/3) matching Ch 1 pattern. Updated build.py (27 fragments). 2 new formulas (#19-#20). Summary Ch 2 card + 3 quick-ref entries. |
